...
public class BoxScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (ScoreManager.instance.GetCurrentScore() <= 50)
        {
            move_Speed = move_Speed + .2f * ScoreManager.instance.GetCurrentScore();
        }
        else
        {
            move_Speed = move_Speed + .2f * 50;
        }
    ...
    void OnCollisionStay2D(Collision2D target) {
        ...
        if (ignoreCollision)
        {
            if (target.gameObject.tag == "Platform")
            {
                ...
                character.GetComponent<CharacterMovement>().isOnPlatform2 = false;
            }
            return;
        }
        ...
        if (target.gameObject.name == "character")
        {
            ...
            character.GetComponent<CharacterMovement>().isOnPlatform2 = true;
        }
